 ...
 * none of which are reachable over HTTP. e2e/post-week-harness.ts drives the real
 * functions against a throwaway database with only the mail senders injected.
 */
test("post week to Slack", async () => {
  test.slow(); // spins up its own database and applies migrations to it

  const { stdout } = await new Promise<{ stdout: string }>((resolve, reject) => {
    execFile(
      "npx",
      ["tsx", path.join(__dirname, "post-week-harness.ts")],
