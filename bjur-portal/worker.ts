...
async function markLoopTick() {
  // One phase again. The marked proxy is both what the gallery streams and what Download
  // hands over, so there is no expensive second encode to keep out of this queue's way.
  const pending = await db.asset.findMany({
