    ...
    <Portal>
      ...
      >
        <div
          ...
          className="w-full max-w-[640px] bg-s2 border border-line2 p-7 bjrise"
        >
          <div className="text-xl font-black tracking-tight mb-1.5">
            Client uploads
          </div>
          <div className="text-[13px] text-muted mb-6">
            Raw footage &amp; files sent in for &ldquo;{projectTitle}&rdquo; —
            each bin below is already browsable at that path over SMB, no
            download needed on the LAN.
          </div>
          ...
          <div className="flex flex-col gap-4 max-h-[480px] overflow-y-auto mb-6">
            ...
            {batches?.map((b) => {
              const totalBytes = b.files.reduce(
                  ...
                  <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-line2 bg-bg/40">
                    <div className="min-w-0">
                      ...
                      <div className="text-[11px] text-dim mt-0.5">
                        ...
                        {formatBytes(totalBytes)} · {b.uploaderName} ·{" "}
                        {fmtDate(b.createdAt)}
                      </div>
                    </div>
                    {allComplete && (
