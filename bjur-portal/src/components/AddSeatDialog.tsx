...
export function AddSeatDialog({
  ...
  async function submit() {
    ...
    const projectAccess = restricted
      ? Array.from(grants.entries()).map(([projectId, r]) => ({ projectId, role: r }))
      : undefined;
    const res = await fetch(`/api/admin/clients/${clientId}/users`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: name.trim(), email: email.trim(), role, projectAccess }),
    });
        ...
        {!result ? (
          <>
            ...
            <div className="flex flex-col gap-4">
              ...
              <Field label={restricted ? "Default role" : "Role"} htmlFor="srole">
                <select
                  id="srole"
                  value={role}
              ...
              {restricted && (
                <div className="border border-line2 max-h-[220px] overflow-y-auto">
                  {projects.map((p) => {
                    ...
                    return (
                      <div key={p.id} className="flex items-center gap-3 px-4 py-2.5 border-b border-line last:border-b-0">
                        <input
                          type="checkbox"
                          checked={checked}
                        ...
                        {checked && (
                          <select
                            ...
                            onChange={(e) =>
                              setGrants((prev) => new Map(prev).set(p.id, e.target.value as Role))
                            }
